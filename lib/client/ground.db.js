...
Ground.Collection = class GroundCollection {
  ...
  constructor(name, {
    ...
    this.storage = storageAdapter || new LocalForage({
      name: name,
  ...
  observeSource(source=this) {
    ...
    this.sourceHandle = cursor.observe({
      'added': doc => {
        ...
        if (this !== source) this.setDocument(doc);
      ...
      'changed': (doc, oldDoc) => {
        ...
        if (this.lastUpdatedAt) {
          if (doc.removedAt && !oldDoc.removedAt) {
            ...
            if (this !== source) this.setDocument(doc, true);
  ...
  keep(cursors) {
    ...
    const keepIds = _.flatten(_.map(arrayOfCursors, (cursor) => cursor.map((doc) => doc._id)));
